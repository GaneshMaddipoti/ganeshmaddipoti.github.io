...
import utils.Constants.{ELEMENT_BG_COLOR, NODE}
import utils.HtmlElementUtil.div

case class NodeX(override val model: GraphElement) extends AbstractElement(model) {
...
object NodeX {
  def NodeModel(e: MouseEvent, parent: Option[AbstractElement]): GraphElement =
    GraphElement(
      id = s"$NODE-${System.currentTimeMillis()}",
